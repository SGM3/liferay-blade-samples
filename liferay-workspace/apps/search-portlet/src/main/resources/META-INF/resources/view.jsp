...
<div class="form-group">
    <form action="<%= portletURL %>" method="POST" name="example">
        ...
        <c:forEach items="${facetsWithAvailableTerms}" var="curFacet">
            ...
            <input name="<portlet:namespace/>${curFacet.key}" type="checkbox" value="${termEntry.term}" <%= userSelectedTermValues.contains(termEntry.getTerm())?"checked":StringPool.BLANK %>> ${termEntry.term} (${termEntry.frequency})<br>
            </c:forEach>
...
<div id="<portlet:namespace/>scDiv" class="collapse">
    <liferay-ui:search-container delta="10">
        ...
        <liferay-ui:search-container-row
            ...
            keyProperty="UID"
            modelVar="doc"
            indexVar="curIndexForDocs" >

            <liferay-ui:search-container-column-text
                name="Entry Number"
                value='<%= (curIndexForDocs + 1) + StringPool.BLANK %>'
            />
